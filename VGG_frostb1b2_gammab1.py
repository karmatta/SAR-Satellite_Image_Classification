...
def gen_image_data(data):
    ...
    X_band_1_frost = np.array([np.array(band).astype(np.float32).reshape(75, 75) for band in data["band_1_frost"]])
    X_band_2_frost= np.array([np.array(band).astype(np.float32).reshape(75, 75) for band in data["band_2_frost"]])
    ...
    X_train = np.concatenate([band_1_g[:, :, :, np.newaxis]
                                 , X_band_1_frost[:, :, :, np.newaxis]
                                 , X_band_2_frost[:, :, :, np.newaxis]], axis=-1)
...
def main():
    ...
    train = pd.read_json('~/Karthik/StatOil/train_trans_band_frost.json')
    test = pd.read_json('~/Karthik/StatOil/test_trans_band_frost.json')
    ...
    submission.to_csv('VGG_frostb1b2_gammab1.csv', index=False)
